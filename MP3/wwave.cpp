...
BOOL WWaveOut::OpenFile(char *filename)
{
...
// RETURN DWORD				- position
    ...
    switch (wType) {
        ...
        case MILLISECONDS:
        	mmt.wType = TIME_SAMPLES;
        	waveOutGetPosition(_whandle, &mmt, sizeof(MMTIME));
        return ( mmt.u.sample * 1000 / _samples_per_sec );
...
//          UINT BufferSize				- size of single output buffer
//
// RETURE:	TRUE - all OK
//			FALSE	- error  
    ...
    _wfx.nBlockAlign =  ( Channels * BitsPerSample ) / 8;
    _wfx.wBitsPerSample = BitsPerSample;
    _wfx.cbSize = 0;
	...
	int size = 0;
	_alloc_buffer_size = _wfx.nAvgBytesPerSec * BufferSize / 1000;
...
// deallocate buffer memory
 ...
 BOOL WWaveOut::IsBufferDone(DWORD index) {  return (_waveHdr[index].dwFlags & WHDR_DONE) > 0 ? 1 : 0;};
